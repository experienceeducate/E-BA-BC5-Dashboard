...
function MobRecruitmentFunnelPage({ filters }) {
  ...
  // `conversionCategory` (confirmed ÷ reached — call-center conversion) still
  // drive the Insights section below unchanged; `progressPct`/`category`
  // (confirmed ÷ target) are a separate, distinct signal that drives the
  // categorisation table's filtering/Status instead — a venue can convert
  // well but still be behind its absolute target, or vice versa, so these
  // aren't interchangeable. No per-venue "assigned" exists anywhere.
  ...
  // venueRows.category), same basis as the district table.
  ...
  const filteredSumVenueTarget = sumBy(filteredVenueRows, "target");
  const filteredVenueProgressPct = filteredSumVenueTarget ? Math.round((1000 * filteredSumVenueConfirmed) / filteredSumVenueTarget) / 10 : null;
  ...
  return (
    <div>
      ...
      <ExecBand num="◆" title="Performance vs target — districts and venues" />
      <State loading={heatmap.loading} error={heatmap.error} empty={!heatmap.loading && districtRows.length === 0 && venueRows.length === 0}>
        ...
        <Grid cols={4}>
          ...
          <KpiTile label="Target (sum)" value={fmtNum(filteredSumVenueTarget)} sub="sum of these venues" tag="REAL" />
          <KpiTile label="Progress on target" value={<span style={{ color: RATE_CATEGORY_COLOR[categorizeRate(filteredVenueProgressPct)] }}>{fmtPct(filteredVenueProgressPct)}</span>} sub="confirmed ÷ target" tag="DERIVED" tone="sim" />
        </Grid>
        <Card title="Venue performance" chip="REAL">
          <DataTable
            columns={[
              ...
              { key: "rate", label: "Rate", align: "right", render: (v, r) => <span style={{ color: RATE_CATEGORY_COLOR[r.conversionCategory], fontWeight: 700 }}>{fmtPct(v)}</span> },
              { key: "progressPct", label: "Progress on target", align: "right", render: (v, r) => <span style={{ color: RATE_CATEGORY_COLOR[r.category], fontWeight: 700 }}>{fmtPct(v)}</span> },
